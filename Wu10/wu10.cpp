...
int main()
{
	...
	myfile.open("E:\\data\\runningtime\\lightweightidentity\\wu10_9_17.txt");
	...
	User aUser(g, p, gateWay.getIds());
	//aUser.setIds(gateWay.getIds());
...
void userAuthTime(Gateway gateWay, User aUser) {
	...
	for (int j = 0; j < sizeof(times) / sizeof(times[0]); j++) {
		...
		cout << "User auth time of  Wu10 " << times[j] << " times is :: " << endl;
		cout << sum / expRunningTime << endl;
		myfile << "User auth time of  Wu10 " << times[j] << " times is :: " << endl;
...
void userRegistrationTime(Gateway gateWay, User aUser) {
	...
	for (int j = 0; j < sizeof(times) / sizeof(times[0]); j++) {
		...
		cout << "User registration time of  Wu10 " << times[j] << " times is :: " << endl;
		cout << sum / expRunningTime << endl;
		myfile << "User registration time of  Wu10 " << times[j] << " times is :: " << endl;
		myfile << sum / expRunningTime << endl;
		sum = 0;
	}
}
...
void userChaningPasswordTime(Gateway gateWay, User aUser) {
	...
	for (int j = 0; j < sizeof(times) / sizeof(times[0]); j++) {
		...
		cout << "User password change time of  Wu10 " << times[j] << " times is :: " << endl;
		cout << sum / expRunningTime << endl;
		myfile << "User password change time of  Wu10 " << times[j] << " times is :: " << endl;
		myfile << sum / expRunningTime << endl;
		sum = 0;
	}
}
